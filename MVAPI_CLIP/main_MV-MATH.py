...
def gen_mask_for_multiple_images_with_attention(model, prs, preprocess, device, tokenizer, image_path_or_pil_images, questions):
    ## Load image
    images = []
    image_pils = []
    for image_path_or_pil_image in image_path_or_pil_images:
        if isinstance(image_path_or_pil_image, str):
            ...
            print(image_path_or_pil_images, type(image_path_or_pil_image))
            raise NotImplementedError
    ...
    with torch.no_grad():
        representation = model.encode_image(image, 
                                            attn_method='head', 
                                            normalize=False)  
                          # get the attention weights from image representation
    ...
    # Calculate similarity between each image's attention_cls and the class_embedding
    attention_cls = attentions[:, 0, 0, :]
    attention_cls = F.normalize(attention_cls, dim=-1)
    cls_similarity = torch.einsum('bd,bd->b', attention_cls, class_embedding)

    # Apply the attention mechanism: Compute attention for each image
    attention_map = attentions[:, 0, 1:, :]
    attention_map = torch.einsum('bnd,bd->bn', attention_map, class_embedding)

    # Reshape attention maps into the desired dimensions
    HW = int(np.sqrt(attention_map.shape[1]))  # Assuming attention map is square
    batch_size = attention_map.shape[0]
    attention_map = attention_map.view(batch_size, HW, HW)

    # We now apply image-specific weights based on similarity scores
    image_weights = F.softmax(cls_similarity*10, dim=0)  # Normalize to get weights

    image_weights_expanded = image_weights.unsqueeze(1).unsqueeze(2)  #  [4, 1, 1]
    image_weights_expanded = image_weights_expanded.expand(-1, 24, 24)  #  [4, 24, 24]

    weighted_attention_map = attention_map * image_weights_expanded  # [4, 24, 24]

    # Get token maps from mlp layers
    token_map = torch.einsum('bnd,bd->bn', mlps[:,0,:,:], class_embedding)
    token_map = token_map.view(batch_size, HW, HW)

    return image_pils, weighted_attention_map, token_map, cls_similarity
...
def blend_mask(image, cls_mask, patch_mask, key, enhance_coe, kernel_size, interpolate_method, grayscale, folder):
    ...
    file_name = os.path.join(folder, f"{key}")
    print('folder:', folder, '----------')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_folder", type=str, default = "your/path/result/multiimage/clip")
    ...
    exp_folder = f"{args.output_folder}/MVAPICLIP_{args.dataset}_{args.model_name}_{args.layer_index}"
    ...
    mask_image_folder = os.path.join(exp_folder, f"{enhance_coe}_{kernel_size}_{interpolate_method_name}_{grayscale}_concise")
    text_folder = os.path.join(mask_image_folder, "")
    os.makedirs(text_folder + "input_image", exist_ok=True)

    total_batches = len(dataset)
    with tqdm(total=total_batches, desc="Processing Batches") as pbar_outer:
        for keys, image_path_or_pil_images, questions, image_paths in dataset_loader:
            ...
            pbar_outer.set_postfix({"len_questions": len(questions), "image_paths": image_paths})
            with torch.no_grad():
                image_pils, cls_masks, patch_masks, cls_similarity = gen_mask_for_multiple_images_with_attention(
                    model, prs, preprocess, device, tokenizer, image_path_or_pil_images[0], questions)
            ...
            for key, image, cls_mask, patch_mask, image_path in zip(keys, image_pils, cls_masks, patch_masks, image_paths[0]):
                with tqdm(total=1, desc=f"Processing {key}", leave=False) as pbar_inner:
                    ...
                    merged_image = blend_mask(image, cls_mask, patch_mask, image_path, enhance_coe, kernel_size, interpolate_method, grayscale, text_folder)
                    pbar_inner.update(1)  
            pbar_outer.update(1)  
