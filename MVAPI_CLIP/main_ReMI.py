...
def merge_mask(cls_mask, patch_mask, kernel_size = 3, enhance_coe = 10):
    ...
    mask = apply_minimum_cutoff(mask, threshold=0.5)
...
def blend_mask(image, cls_mask, patch_mask, key, enhance_coe, kernel_size, interpolate_method, grayscale, folder):
    ...
    file_name = os.path.join(folder, os.path.basename(key))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="ReMI")
    parser.add_argument("--model_name", type=str, default="ViT-L-14-336")
    parser.add_argument("--batch_size", type=int, default=1)
    parser.add_argument("--layer_index", type=int, default=15)
    parser.add_argument("--range", type=int, nargs = 2, default=(0,3000))
    parser.add_argument("--output_folder", type=str, default = "your/path/ReMI/clipcutoff0.5")
    ...
    mask_image_folder = os.path.join(mask_image_folder, "images")
    os.makedirs(mask_image_folder + "/images", exist_ok=True)
    ...
    with tqdm(total=total_batches, desc="Processing Batches") as pbar_outer:
        for keys, image_path_or_pil_images, questions, image_paths in dataset_loader:
            ...
            if len(image_paths[0]) > 8:
                print(f"Skipping batch with {len(image_paths[0])} images, as it exceeds the limit of 9.")
                continue
            ...
            with torch.no_grad():
                image_pils, cls_masks, patch_masks = gen_mask(model, prs, preprocess, device, tokenizer, image_path_or_pil_images[0], questions)
            ...
            for key, image, cls_mask, patch_mask, image_path in zip(keys, image_pils, cls_masks, patch_masks, image_paths[0]):
                with tqdm(total=1, desc=f"Processing {key}", leave=False) as pbar_inner:
                    merged_image = blend_mask(image, cls_mask, patch_mask, image_path, enhance_coe, kernel_size, interpolate_method, grayscale, mask_image_folder)
                    pbar_inner.update(1) 
            pbar_outer.update(1)  